...
static int is_dead_end( int state){
	if ((state & PEASANT) == 0){
		if ((state & WOLF) && (state & GOAT)){
			return 1;
		}else if ((state & CABBAGE) && (state & GOAT)){
			return 1;
		}
	}else{
		if (!(state & WOLF) && !(state & GOAT)){
...
static int is_possible_transition( int state1,	int state2){
	...
	if (changeP(state1) == state2 ||
	changePW(state1) == state2 ||
	changePG(state1) == state2 ||
	changePC(state1) == state2
	){
		return 1;
	}
...
static int changePW( int state){
	if ((state & PEASANT) / PEASANT != (state & WOLF) / WOLF)
		return -1;
	else
...
static int changePG( int state){
	if ((state & PEASANT) / PEASANT != (state & GOAT) / GOAT)
		return -1;
	else
...
static int changePC( int state){
	if ((state & PEASANT) / PEASANT != (state & CABBAGE) / CABBAGE)
		return -1;
	else
...
static void print_path( int visited[], int depth){
	for (int i = 0; i <= depth; i++){
		print_statename(NULL, visited[i]);
		printf("\n");
	}
}
...
static void dfs_main( int initial_state, int goal_state, int depth, int visited[]){
	...
	print_statename(NULL, initial_state);
	printf(" (depth %d)\n", depth);
	...
	next_state[0] = changeP(initial_state); next_state[1] = changePW(initial_state);
	next_state[2] = changePG(initial_state); next_state[3] = changePC(initial_state);
	
	for (int i = 0; i < 4; i++){
		if (next_state[i] != -1){
			if (is_visited(visited, depth, next_state[i])){
				printf("\tnext state ");
				print_statename(NULL, next_state[i]);
				printf(" has been visited\n");
			}else if (!is_possible_transition(initial_state, next_state[i])){
				printf("\tnext state ");
				print_statename(NULL, next_state[i]);
				printf(" is dead-end\n");
			}else{
				dfs_main(next_state[i], goal_state, depth + 1, visited);
