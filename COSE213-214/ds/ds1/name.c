...
void load_names_lsearch(FILE* fp, int year_index, tNames* names) {
	...
	while (!feof(fp)) {
		...
		*ptr = '\t';
		...
		sscanf(buffer, "%s\t%c\t%d", name, &sex, &freq);
		...
		if (year_index == 0) {
			...
			if (result == -1) {
				...
				names->data[result].freq[year_index] = freq;
			}
		}
...
void load_names_bsearch(FILE* fp, int year_index, tNames* names) {
	...
	while (!feof(fp)) {
		...
		*ptr = '\t';
		...
		input.sex = sex;
		tName* result = bsearch(&input, names->data, origin_len, sizeof(tName), compare);

		if (result != NULL) {
			result->freq[year_index] = freq;
		}
		else {
			memset(names->data[names->len].freq, 0, MAX_YEAR_DURATION * sizeof(int));
